...
public class ServiceAccountEndToEndTests(ITestOutputHelper testOutput)
{
  ...
  public async Task ServiceAccount_EndToEnd_CompletesFullProvisioningFlow()
  {
    ...
    var createTenantReq = new V1Dtos.CreateTenantRequestDto("EndToEnd Tenant");
    ...
    var tenantResult = await tenantResponse.Content
      .ReadFromJsonAsync<V1Dtos.CreateTenantResponseDto>(TestContext.Current.CancellationToken);
    ...
    var keyResult = await keyResponse.Content
      .ReadFromJsonAsync<V1Dtos.CreateInstallerKeyResponseDto>(TestContext.Current.CancellationToken);
